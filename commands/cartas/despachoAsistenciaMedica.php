<?php
class despachoAsistenciaMedica extends sessionCommand{
	function execute(){
		...
		if($this->request->idPoliza){
			...
			$personas = Fabrica::getAllFromDB("Vehiculo", array("idPoliza = '" . $poliza->getId() . "'"));
			if(count($vehiculos)=="1"){
				$modelo = Fabrica::getFromDB("Modelo", $vehiculos[0]->getIdModelo());
				if($vehiculos[0]->getIdModelo()!=""){
					$marca = Fabrica::getFromDB("Marca", $modelo->getIdMarca());
					$this->addVar("marca", $marca->getMarca());
				}else{
					$this->addVar("marca", "Sin Definir");
				}
				$this->addVar("placa", $vehiculos[0]->getPlaca());				
			}
			...
			$this->processTemplate("cartas/despachoAsistenciaMedica.html");
		}
	}
}
?>