...
jQuery(function($){
    $.fn.fcbkcomplete = function(opt){
        return this.each(function(){
            ...
            function addUniqueItem(title, value, preadded, locked, focusme) {
                var splitted = value.split("|");
                var value = splitted[0];
                var itemtxt = splitted[1] ? splitted[1] : "";
                var li = document.createElement("li");
                var txt = document.createTextNode(itemtxt.length > 0 ? itemtxt : title);
                ...
                $(aclose).click(function(){
                    ...
                    holder.removeClass("unique-holder");
                    addInput(true);
                    return false;
                });

                holder.addClass("unique-holder");

                if (!preadded) {
                    //$("#" + elemid + "_annoninput").remove();
                    holder.find(".bit-input").remove();
                    var _item;
            ...
            function addItem(title, value, preadded, locked, focusme){
                if (options.onlyone) {
                    return addUniqueItem(title, value, preadded, locked, focusme);   
                }
                var splitted = value.split("|");
                var value = splitted[0];
                var itemtxt = splitted[1];
                if (!maxItems()) {
                    return false;
                }
                var li = document.createElement("li");
                ...
                input.keyup(function(event){
                    ...
                    if (event.keyCode != 40 && event.keyCode != 38 && etext.length != 0) {
                        ...
                        if (options.json_url) {
                            ...
                            addMembers(etext, options.callback());
                            bindEvents();
                        }
                        else {
                            addMembers(etext);
            ...
            function addMembers(etext, data){
                ...
                while (match != null && maximum > 0) {
                    ...
                    if (options.filter_selected && element.children("option[value=" + object.value + "]").hasClass("selected")) {
                        ...
                        content += '<li rel="' + object.value + '|' + object.item + '">' + itemIllumination(object.caption, etext) + '</li>';
                        counter++;
                        maximum--;
                    }
                    match = myregexp.exec(search_string);
...
//                var maininput = $("#" + elemid + "_annoninput").children(".maininput");
                var maininput = holder.find(".maininput");
                bindFeedEvent();
            ...
            var options = $.extend({
                ...
                filter_case: false,
                filter_hide: false,
                complete_text: "Empieza a escribir...",
                maxshownitems: 30,
                maxitems: 10,
                onselect: "",
                onremove: "",
