...
class Request{
	...
	function __construct($getQuery){
		if($getQuery){
			//inicializando objeto Request a partir de string
			//con formato do=action&firstName=John&lastName=Doe
			$getPieces=explode("&",$getQuery);
			$getVars=array();
			foreach($getPieces as $gp){
				$gpKeyVal=explode("=",$gp);
				if(sizeof($gpKeyVal)!=2){
					$msg="Invalid request format: ".$getQuery;
					new Error($msg);
				}
				$getVars[$gpKeyVal[0]]=$gpKeyVal[1];
			}
